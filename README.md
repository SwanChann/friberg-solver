...
- 对局推演：为猜测选手录入 8 项绿/黄/灰反馈和数字方向，多轮条件始终以 AND 从完整数据集重新计算。
...
- 不包含登录、云数据库、AI API、OCR、常驻爬虫或自动作弊功能。
...
测试覆盖国家/赛区、年龄边界和箭头、Major 数值边界、角色、当前队伍、现役、所有数值筛选运算符、多 Guess AND、删除 Guess 后恢复、指定人工筛选案例、全表列排序、数据 schema 以及信息增益输出。历史队伍规则只保留合成数据兼容测试，不作为内置数据功能验收项。
...
内置快照没有历史队伍，因此队伍反馈只显示绿色/灰色，不提供无法可靠推演的黄色。若以后导入带非空 `team_history` 的数据，该兼容能力会自动启用。
...
- 原游戏单人模式上限为 8 次。本 Solver 显示 `Guess n / 8`，但不强制阻止继续推演。
...
src/components/       filters, candidates, feedback editor, dataset info
